...
export default function ChangePassword() {
    ...
    const handleChangePassword = async () => {
        ...
        try {
            ...
            const response = await axios.post(`http://localhost:3000/auth/updatePassword/${username}`, reqBody, {
                headers: {
            ...
            console.log(error);
            toast.error(error.response.data.message ? error.response.data.message : "Error while changing password");
            ...
            <CardContent className="space-y-4 mt-4">
                ...
                <div className="relative space-y-3">
                    ...
                    <Input
                        ...
                        className="text-gray-600"
                    />
                    <Button variant="ghost" size="icon" className="absolute bottom-2.5 right-1 h-7 w-7" onClick={toggleNewPasswordVisibility}>
                        {showNewPassword ? <AiOutlineEyeInvisible className='h-4 w-4'/> : <AiOutlineEye className='h-4 w-4'/>}
                        <span className="sr-only">Toggle password visibility</span>
                    </Button>
                </div>
                <div className="relative space-y-3">
                    ...
                    <Input
                        ...
                        className="text-gray-600"
                    />
                    <Button variant="ghost" size="icon" className="absolute bottom-2.5 right-1 h-7 w-7" onClick={toggleConfirmPasswordVisibility}>
                ...
                <div className="flex flex-col justify-center">
                    {errors.map((error, index) => (
                        <div key={index} className="text-red-500 font-medium">{error}</div>
                    ))}
                </div>
            </CardContent>
            <CardFooter>
