    ...
    <Dialog
        ...
        >
            <div className='flex text-sm font text-gray-500'>
                <h3 className='flex items-center justify-center cursor-pointer hover:text-black py-4 pl-4'>View all submissions <ChevronRight className="h-4 w-4 pt-1" /></h3>
            </div>
        </DialogTrigger>
        <DialogContent className='max-w-7xl h-[95%] my-auto w-full'>
            <div className='overflow-auto'>
                <div className='flex text-xl font-semibold items-center justify-center select-none'>
                    <h3 className={`flex items-center justify-center cursor-pointer hover:text-black gap-2 pb-4 px-4`}><GoChecklist className='w-7 h-6' />All Submissions</h3>
                </div>
                {items && items.length > 0 
                    ?   (<div>
                            {items.map((item, index) => {
                                ...
                                return (
                                <div className={`flex items-center justify-between p-5 rounded-md ${isGray ? 'bg-gray-100' : ''}`} key={index}>
