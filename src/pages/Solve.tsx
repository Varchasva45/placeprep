  ...
  const Solve = () => {
    ...
    useEffect(() => {
        ...
        const fetchProblem = async () => {
            try {
            const response = await axios.get(`http://localhost:3000/problem/api/problems/${id}`);
        ...
        <ResizablePanel defaultSize={40}>  
          <div className="min-w-96">
            <div className="flex flex-col gap-5 m-3">
                <h1 className=" text-3xl font-bold">{questionId}. {title}</h1>
                <h2 className="text-2xl font-bold">{difficulty}</h2>
                <p className="text-gray-700 text-lg">{description}</p>
                <Accordion type="single" collapsible className="w-full">
                <AccordionItem value="item-1">
                <AccordionTrigger><h2 className="text-2xl font-bold">Topics</h2></AccordionTrigger>
                <AccordionContent>   
