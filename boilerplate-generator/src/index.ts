...
function main() {
  fs.readdir('./problems' || "", (err, files) => {
    files.forEach(file => {
      if (file)
        generatePartialBoilerplate(path.join('./problems' || "", file));
      generateFullBoilerPLate(path.join('./problems' || "", file));
    })
  })
}
if (!'./problems') {
  console.log("Store a valid problems dir path in .env", './problems');
} else {
  getFolders('./problems').then((folders: any) => {
    folders.forEach((folder: string) => {
      generatePartialBoilerplate(path.join('./problems' || "", folder));
      generateFullBoilerPLate(path.join('./problems' || "", folder));
    });
  })
}
main();